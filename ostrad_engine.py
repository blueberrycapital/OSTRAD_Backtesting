...
def setup_logging(log_file: str = 'ostrad_backtest.log'):
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_file, mode='a', encoding='utf-8'),  # Add encoding='utf-8'
...
class CSVDataLoader(DataLoader):
    ...
    def _load_files(self):
        # Load spot data - EXACTLY like your original working code
        ...
        # Rename 'index' column to 'symbol' - this is what your original code did
        # But check if 'index' exists first
        if 'index' in self.spot_df.columns:
            self.spot_df = self.spot_df.rename(columns={'index': 'symbol'})
        
        # Select columns - use 'symbol' if it exists, otherwise use what's available
        ...
        # Load options data - EXACTLY like your original working code
        ...
        # Select columns - include 'oi' if present
    ...
    def load_data(self, symbol: str, start_date: datetime, end_date: datetime) -> Dict[str, pd.DataFrame]:
        ...
        # Filter by symbol using 'symbol' column
...
class InfluxDBDataLoader(DataLoader):
    ...
    Preloads data once, then filters by date range in load_data().
    """
    ...
    def __init__(self, url: str, token: str, org: str,
        ...
        self.token = token
        self.org = org
        # Support separate spot/option buckets or a single shared bucket
    ...
    def _load_data(self, start_date: datetime, end_date: datetime):
        """Load and preprocess all data from InfluxDB - mirrors CSVDataLoader._load_files()"""
...
'''
        ...
        if not self.options_df.empty:
            ...
            # Rename 'index' → 'symbol' (prefer over 'trading_symbol')
            ...
            # Rename 'expiry_date' → 'expiry' only when 'expiry' doesn't already exist
...
class OSTRADBacktester:
    ...
    def _calculate_summary(self) -> Dict:
        ...
        # Calculate from closed_positions instead of trade_log
        all_pnls = [p.pnl for p in self.closed_positions if p.exit_time is not None]
